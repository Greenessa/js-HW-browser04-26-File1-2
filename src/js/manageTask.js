...
export default class ManageTask {
  ...
  addListeners() {
    ...
    this.containerEl.addEventListener("mousedown", (e) => {
      ...
      if (e.target.classList.contains("cross")) {
        ...
        if (hEl.textContent === "TODO") {
          this.currentState.taskArrayToDo =
            this.currentState.taskArrayToDo.filter(
              (item) => item !== removeEl.textContent,
            );
        } else if (hEl.textContent === "PROGRESS") {
          this.currentState.taskArrayProgress =
            this.currentState.taskArrayProgress.filter(
              (item) => item !== removeEl.textContent,
            );
        } else {
          this.currentState.taskArrayDone =
            this.currentState.taskArrayDone.filter(
              (item) => item !== removeEl.textContent,
            );
        }
        removeEl.remove();
    ...
    this.containerEl.addEventListener("mousemove", (event) => {
      ...
      let underMouseEl = document.elementFromPoint(
        event.clientX,
        event.clientY,
      );
      ...
      if (underMouseEl.classList.contains("task")) {
        ...
        if (!checkEl) {
          ...
          this.ghostEl.hidden = true;
          const upTask = document.elementFromPoint(
            event.clientX,
            event.clientY - this.gapTask,
          );
          const downTask = document.elementFromPoint(
            event.clientX,
            event.clientY + this.gapTask,
          );
          this.ghostEl.hidden = false;
